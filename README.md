...
- **34 figures**, all generated: Lambert conformal conic maps drawn from Natural
  Earth, the generational ladder, the register comparison, the evidence chain.
- **178 numbered notes**, renumbered into document order at build time.
...
`qa_ov.py` walks every `<text>` node in all 34 figures, takes its `getBBox()`, and
